...
#if defined(NDEBUG)
# error "Taggchain cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe444dd66cda9b3e2c38322559ef9db63bdc4e7d0408984d3255933601d00fa6b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Taggchain: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Taggchain Signed Message:\n";
...
{
    // Taggchain: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 60; // Taggchain: 1 minute
static const int64 nTargetSpacing = 1 * 60; // Taggchain: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Taggchain: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Taggchain: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe444dd66cda9b3e2c38322559ef9db63bdc4e7d0408984d3255933601d00fa6b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 27/Nov/2017 Confessions of a Digital Nazi Hunter";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042731810fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1511749118;
        ...
        assert(block.hashMerkleRoot == uint256("0x8aafc2a5f8f8831442dda7e0758906a59f0c615fdcb07291f2f899c0c019dae7"));
...
unsigned char pchMessageStart[4] = { 0xcf, 0xd6, 0xd4, 0xb3 }; // Taggchain: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TaggchainMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TaggchainMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TaggchainMiner(CWallet *pwallet)
{
    printf("TaggchainMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("taggchain-miner");
    ...
    try { loop {
        ...
        printf("Running TaggchainMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TaggchainMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TaggchainMiner, pwallet));
}
